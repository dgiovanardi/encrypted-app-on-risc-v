/*
    RISC-V RV32I Emulator for RSA crypted code
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
   ...
   RISC-V opcodes decipher

 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

RiscV_Decipher::RiscV_Decipher()
    : RiscV()
{
    FCachedPC = 0xffffffff; // Invalid PC
    FCachedInstruction = 0; // Invalid instruction
}
//---------------------------------------------------------------------------

unsigned long RiscV_Decipher::getInstruction()
{
TPlainOpCodeSlices   slPlain;
TCryptedOpCodeSlices slCrypt;

    // Bypass decryption if instruction cached
    if (PC == FCachedPC)
        return FCachedInstruction;

    // Assume plain code if N is NULL
    if (!Decipher.N)
        return inherited::getInstruction();

    // RSA decrypt
    slCrypt.CrypedOpCode = CipheredInstruction;
    for (int c=0; c<4; c++)
        slPlain.Bytes[c] = Decipher.Decrypt[slCrypt.Shorts[c]];

    // Restore plain opcode by XORing entropy generated by nonce
    RandSeed = PC; // Nonce
    slPlain.OpCode ^= Random(0xffffffff);

    // Cache instruction
    FCachedPC = PC;
    FCachedInstruction = slPlain.OpCode;

    return slPlain.OpCode;
}
//---------------------------------------------------------------------------

unsigned __int64 RiscV_Decipher::getCipheredInstruction()
{
    // .text segment is doubled so PC points to the "half" of the offset
    return *(unsigned __int64 *)(pMemory + (PC-minText)*sizeof(short) + minText);
}
//---------------------------------------------------------------------------
