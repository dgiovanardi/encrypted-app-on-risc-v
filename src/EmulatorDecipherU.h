...
// Helper struct to encrypt/decrypt OpCodes
typedef union
{
...
typedef union
{
    unsigned __int64 CrypedOpCode;
    unsigned short   Shorts[4];
} TCryptedOpCodeSlices;
...
protected:
    ...
    __property          char *pMemory        = { read=FpMemory };
    __property unsigned long  minText        = { read=FminText };
...
private:
    // Cache last decrypted insn to speed up the execution
    unsigned long FCachedPC;
    unsigned long FCachedInstruction;
...
public:
    ...
    __property unsigned __int64 CipheredInstruction = { read=getCipheredInstruction };
};
//---------------------------------------------------------------------------
